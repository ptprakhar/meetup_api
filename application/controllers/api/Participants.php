...
class Participants extends REST_Controller {
    ...
    ***/
    public function index_get($offset = 0, $limit = 10) {
        ...
        if (!empty($participantsData)) {
            ...
            $this->response(array("status" => true, "message" => "Data Found :: Total Object(s) " . count($participantsData), "data" => [$participantsData]), REST_Controller::HTTP_OK);
        } else {
            //If there is no data in database
            $this->response(array("status" => false, "message" => "No data found", "data" => []), REST_Controller::HTTP_OK);
        }
    }
    /***
    method :: POST
    ***/
    public function index_post($limit = 0, $offset = 10) {
        ...
        if ($this->form_validation->run() === false) {
            // we have some errors
            $this->response(array("status" => false, "message" => "validations failed, please enter data in correct format "), REST_Controller::HTTP_OK);
        } else {
            //Validations passed Insert the data
            $insertData = array('name' => $name, 'age' => $age, 'dob' => convertToDate($dob), 'profession' => $profession, 'locality' => $locality, 'no_of_guests' => $no_of_guests, 'address' => $address);
            if ($this->ParticipantModel->insert_data('participants', $insertData)) {
                //Sucessfully Inserterd
                $this->response(array("status" => true, "message" => "Record has been created!"), REST_Controller::HTTP_OK);
            } else {
                //Not inserted, some error found while inserting
                $this->response(array("status" => false, "message" => "Some error in inseting the data! Please Try Again"), REST_Controller::HTTP_OK);
            }
        }
    }
    /***
    ...
    **/
    public function index_put($id) {
        ...
        //checking form submittion have any error or not
        if ($this->form_validation->run() == false) {
            $this->response(array("status" => true, "message" => "Validations failed!"), REST_Controller::HTTP_NOT_FOUND);
        } else {
            if (isset($id) && $id > 0) {
                //Check if data is present in DB or not
                if ($this->ParticipantModel->checkDataById($id) == false) {
                    //No Data in Databse
                    $this->response(array("status" => false, "message" => "Trying to update the record for invalid ID!"), REST_Controller::HTTP_NOT_FOUND);
                    //exit();
                    
                } else {
                    $updateData = array('name' => $name, 'age' => $age, 'dob' => convertToDate($dob), 'profession' => $profession, 'locality' => $locality, 'no_of_guests' => $no_of_guests, 'address' => $address);
                    if ($this->ParticipantModel->update_data('participants', $id, $updateData)) {
                        //Sucessfully Updated
                        $this->response(array("status" => true, "message" => "Record has been updated!"), REST_Controller::HTTP_OK);
                    } else {
                        //Not updated, some error found while inserting
                        $this->response(array("status" => false, "message" => "Some error in updating the data! Please Try Again"), REST_Controller::HTTP_OK);
                    }
                }
            } else {
